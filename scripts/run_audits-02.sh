...
NO_WEBHOOK_RESPONSE="${NO_WEBHOOK_RESPONSE:-false}"
...
usage() {
  cat <<'EOF'
Usage:
  ./run_audits-02.sh [options]

Options:
  --slug <slug>                Run only one business
  --webhook-url <url>          Send async results to webhook
  --no-webhook-response        Return inline response
  -h, --help                   Show help

Examples:
  ./run_audits-02.sh
  ./run_audits-02.sh --slug crema-downtown
  ./run_audits-02.sh --no-webhook-response
  ./run_audits-02.sh --webhook-url https://example.com/webhook
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --webhook-url)
      WEBHOOK_URL="$2"
      NO_WEBHOOK_RESPONSE="false"
      shift 2
      ;;
    --no-webhook-response)
      NO_WEBHOOK_RESPONSE="true"
      shift
      ;;
    -h|--help)
      usage
...
if [[ "$NO_WEBHOOK_RESPONSE" != "true" && -z "$WEBHOOK_URL" ]]; then
  echo "Error:"
  echo "You must either:"
  echo "  - set NO_WEBHOOK_RESPONSE=true"
  echo "  - or provide WEBHOOK_URL"
  exit 1
fi
...
run_one() {
  ...
  if [[ "$NO_WEBHOOK_RESPONSE" == "true" ]]; then
